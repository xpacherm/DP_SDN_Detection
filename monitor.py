...
class SimpleMonitor15(simple_switch_15.SimpleSwitch15):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields = {'time':'', 
                       ...
                       'psh_flags':0, 
                       'urg_flags':0}
        self.psh_flag_counts = {}
        self.urg_flag_counts = {}
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        for flow in body: 
            if (flow.table_id == 0 and flow.priority > 0):
                dict_key = hash(','.join([str(flow.match['in_port']), str(flow.match['tcp_src']), str(flow.match['tcp_dst'])]))
                self.psh_flag_counts[dict_key] = flow.stats['packet_count']
            if (flow.table_id == 1 and flow.priority > 0):
                dict_key = hash(','.join([str(flow.match['in_port']), str(flow.match['tcp_src']), str(flow.match['tcp_dst'])]))
                self.urg_flag_counts[dict_key] = flow.stats['packet_count']
        ...
        for stat in sorted([flow for flow in body if flow.priority > 3 and flow.table_id == 10], key=lambda flow: (flow.match['in_port'], flow.match['eth_dst'])):
            ...
            if stat.priority == 5: #TCP
                ...
                dict_key = hash(','.join([str(stat.match['in_port']), str(stat.match['tcp_src']), str(stat.match['tcp_dst'])]))
                if dict_key in self.psh_flag_counts:
                    self.fields['psh_flags'] = self.psh_flag_counts[dict_key]
                if dict_key in self.urg_flag_counts:    
                    self.fields['urg_flags'] = self.urg_flag_counts[dict_key]
